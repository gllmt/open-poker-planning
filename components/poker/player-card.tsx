...
export function PlayerCard({ game, player, currentPlayerId }: { game: Game; player: Player; currentPlayerId: string }) {
  const canRemove =
    isModerator(game.createdById, currentPlayerId, game.isAllowMembersToManageSession) && player.id !== currentPlayerId;
...
function getCardColor(game: Game, value: number | undefined): string {
  if (game.gameStatus === Status.Finished) {
    const card = (game.cards?.length ? game.cards : getCards(game.gameType)).find((c) => c.value === value);
...
function getCardDisplayValue(game: Game, cardValue: number | undefined): string {
  const cards = game.cards?.length ? game.cards : getCards(game.gameType as GameType);
