...
import { getRecentPlayerName, setRecentPlayerName, upsertPlayerGame } from '@/lib/browser-storage';
...
export function CreateGame() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (gameType === GameType.Custom) {
      const count = customOptions.reduce((acc, option) => (option?.trim() ? acc + 1 : acc), 0);
      if (count < 2) {
