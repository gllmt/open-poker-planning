...
async function jsonOrThrow<T>(res: Response): Promise<T> {
  ...
  if (!res.ok) {
    ...
    const message = (typeof errorValue === 'string' ? errorValue : null) || `Request failed (${res.status})`;
...
export async function createGame(payload: NewGame): Promise<{ gameId: string; joinToken: string; playerId: string }> {
...
export async function joinGame(gameId: string, token: string, playerName: string): Promise<{ playerId: string }> {
...
export async function fetchGameState(params: { gameId: string; token?: string; playerId?: string }): Promise<{ game: Game; players: Player[] }> {
  const url = new URL(`/api/games/${encodeURIComponent(params.gameId)}`, window.location.origin);
...
export async function vote(gameId: string, playerId: string, value: number, emoji?: string) {
...
export async function removePlayer(gameId: string, playerId: string, callerPlayerId?: string) {
...
export async function updateStory(gameId: string, storyName: string, callerPlayerId: string) {
...
export async function setAutoReveal(gameId: string, autoReveal: boolean, callerPlayerId?: string) {
  const res = await fetch(`/api/games/${encodeURIComponent(gameId)}/auto-reveal`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
export async function updateTimer(gameId: string, timerProps: unknown, callerPlayerId?: string) {
